<template>
  <div class="goods">
    ...
    <fooddetails :food="selectedFood" ref="food" @add="ballAdd"></fooddetails>
  </div>
</template>
<script>
      ...
      this.$http.get('/api/goods').then((response) => {
        ...
        if (response.erron === ERR_OK) {
          ...
          this.$nextTick(() => {
            ...
            let goodsEl = document.getElementsByClassName('goods')
            let headerHeight = document.getElementsByClassName('header')[0].offsetHeight
            let tabHeight = document.getElementsByClassName('tab')[0].offsetHeight
            let goodsHeight = headerHeight + tabHeight
            goodsEl[0].style.top = goodsHeight + 'px'
          })
        }
      })
      // let h = document.getElementsByClassName('menu-wrapper')
      // g.style.top = h[0].offsetHeight - 300 + 'px'
      // var one = document.querySelector(".goodfather").offsetHeight
      // document.querySelector(".goods").style.height = one + 'px'
    },
    methods: {
      _initScroll () {
